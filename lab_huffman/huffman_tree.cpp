...
HuffmanTree::removeSmallest(queue<TreeNode*>& singleQueue, queue<TreeNode*>& mergeQueue)
{
     ...
     */
     TreeNode * data = singleQueue.front();
     	// your code!
     	if (mergeQueue.empty() && singleQueue.empty()) {return NULL;}
     		else if (mergeQueue.empty() && !singleQueue.empty()) {
     			data = singleQueue.front();
     			singleQueue.pop();
     			return data;
     		}
     	else {
     		if (!mergeQueue.empty() && singleQueue.empty()) {
     			data = mergeQueue.front();
     			mergeQueue.pop();
     			return data;
     		}
     		else {
     ...
     */
     ...
     for (unsigned long i = 0; i < frequencies.size(); i++) {
   	...
   	while (mergeQueue.size() + singleQueue.size() != 1) {
   		...
   		TreeNode* next = removeSmallest(singleQueue, mergeQueue);
      int freq = smallest->freq.getFrequency()+next->freq.getFrequency();
   		TreeNode* parent = new TreeNode(freq);
   		parent->left = smallest;
   		parent->right = next;
   		mergeQueue.push(parent);
   	}
   	if (singleQueue.empty()) {root_ = mergeQueue.front();}
     ...
     */
     while (bfile.hasBits()) {
   		if (bfile.getNextBit() == 1) {
   			...
   			TreeNode* inner = new TreeNode(0); //set freq to 0 because not a real node for data
   			inner->left = HuffmanTree::readTree(bfile);
   			inner->right = HuffmanTree::readTree(bfile);
   			return inner;
   		}
   	}
   	return NULL;
