...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      // which means you're changing the image directly.  No need to `set`
      // the pixel since you're directly changing the memory of the image.
      int dist_x = x-centerX;
      int dist_y = y-centerY;
      int dist_center = sqrt((dist_x*dist_x)+(dist_y*dist_y));

      if (dist_center > 160) {
        pixel.l = pixel.l*.2;
      }
      else {
        double decrease = dist_center*.005;
        pixel.l = pixel.l-(pixel.l*decrease);
      }
    }
  }
  return image;
  ...
  for (unsigned x = 0; x < secondImage.width(); x++) {
    for (unsigned y = 0; y < secondImage.height(); y++) {
      ...
      if (secondImagepixel.l==1) {
        firstImagepixel.l=firstImagepixel.l+.2;
      }
    }
  }
  return firstImage;
