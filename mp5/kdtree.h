...
class KDTree
{
  ...
  private:
     ...
     */
     Point<Dim> findNNHelper(KDTreeNode* curr, Point<Dim> best, Point<Dim> target, double radius, int dims) const;
     double getRadius(const Point<Dim>& Point1, const Point<Dim>& Point2) const;
     bool intersects(const int dims, const Point<Dim>& best, const Point<Dim>& target) const;
     int partition(int l, int r, int d, int pivot);
     Point<Dim> quickie(int mid, int l, int r, int d);
     typename KDTree<Dim>::KDTreeNode* recursiveKDTree(int d, int l, int r);
     void destroy(KDTreeNode *node);
     typename KDTree<Dim>::KDTreeNode* copy(const KDTreeNode* curr);
