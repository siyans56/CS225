...
{
    /* Your code goes here! */
    string word1up = word1, word2up = word2;
    std::transform(word1.begin(), word1.end(), word1up.begin(), ::toupper);
    std::transform(word2.begin(), word2.end(), word2up.begin(), ::toupper);

    //have a dictionary of a pronunciation key and all words that have it as vals

    if (dict.find(word1up) == dict.end() || dict.find(word2up) == dict.end()) return false;
    else return (dict.find(word1up)->second == dict.find(word2up)->second);
