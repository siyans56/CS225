...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    parent->children.insert(child_itr, new_right); //put blank child into parent
    DataPair mid_elem = *mid_elem_itr;
    parent->elements.insert(elem_itr, mid_elem); //fill blank child
    new_right->elements.assign(mid_elem_itr + 1, child->elements.end()); //take second half
    new_left->elements.assign(child->elements.begin(), mid_elem_itr); //take first half


    if (child->children.empty() == false) { //if it is not empty, odd number
      new_right->children.assign(mid_child_itr, child->children.end()); //take one more  than before
      new_left->children.assign(child->children.begin(), mid_child_itr); //take one less than before
    }
...
/*
    ...
    insert(child, pair);
    if (subroot->elements.size() > order-1) {
